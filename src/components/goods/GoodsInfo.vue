				...
				<div class="mui-card-content">
					<div class="mui-card-content-inner">
                        ...
                        <p>
                            ...
                            <!-- 2.由于涉及到了父子组件的嵌套了，所以，无法直接在goodsinfo页面中获取选中的商品数量值 -->
...
export default {
    ...
    methods:{
        ...
        enter(el,done){
            ...
            //小球动画优化思路：
            //1.先分析导致 动画不准确的本质原因：我们把小球最终位移到的位置，已经局限
            //在了某一分辨率下的 滚动条未滚动的情况下；
            //2.只要分辨率和测试的时候不一样，或者滚动条有一定的滚动距离之后，问题就出现了；
            //3.因此，我们经过分析，得到结论：不能把位置的衡中坐标直接写死了，而是应该
            //根据不同的情况，动态计算这个坐标值；
            //4.经过分析，得出解题思路：先得到徽标的横纵坐标，再得到小球的横纵坐标，然后
            //让y值求差，x值也求差，得到的结果，就是横纵坐标要位移的距离
            //5.如何获取徽标和小球的位置？ 使用 domObject.getBoundingClient Rect()
            ...
            //ease是一种动画效果，cubic-bezier也是一种动画效果
